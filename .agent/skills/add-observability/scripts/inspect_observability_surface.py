...
def main() -> int:
    ...
    result = {
        ...
        "main_mentions_tracing": contains(main_py, "tracing") or contains(main_py, "FastAPIInstrumentor"),
        "has_compose_file": any(path.is_file() for path in compose_files),
        "has_prometheus_asset": any(path.is_file() and "prometheus" in path.read_text(encoding="utf-8") for path in compose_files),
    }
